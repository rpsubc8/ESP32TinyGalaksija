...
//OSD 32x16
 ...
 OSDprintText("ESP32 GALAKSIJA (port by ackerman)",xTextTitle,yTextTitle,ESP32_BLACK,ESP32_WHITE,0,0); //Titulo          
...
void ShowTinyLoadGTPMenu()
{
 ...
 if (gb_language_en==1)
 {
  aSelNum = ShowTinyMenu(" GTP (TAPE)",(char **)gb_loadselect_menu_en,0,max_gb_loadselect_menu);
 }
 else
 {
  aSelNum = ShowTinyMenu("GTP (CINTA)",(char **)gb_loadselect_menu_sp,0,max_gb_loadselect_menu);
 }
 ...
 if (gb_language_en==1)
 {
  if (aLeer==1)
  {
   aSelNum = ShowTinyMenu("  LOAD GTP",(char **)gb_list_gtp_title,gb_gtp_id_sel_cur,max_list_gtp);
  }
  else
  {
   aSelNum = ShowTinyMenu(" SELECT GTP",(char **)gb_list_gtp_title,gb_gtp_id_sel_cur,max_list_gtp);      
  }
 }
 else
 {
  if (aLeer==1)
  {
   aSelNum = ShowTinyMenu("LEER GTP",(char **)gb_list_gtp_title,gb_gtp_id_sel_cur,max_list_gtp);     
  }
  else
  {
   aSelNum = ShowTinyMenu(" SEL GTP",(char **)gb_list_gtp_title,gb_gtp_id_sel_cur,max_list_gtp);     
  }
 }         
...
void ShowTinyLoadGALMenu()
{
 ...
 if (gb_language_en==1)
 {
  aSelNum = ShowTinyMenu("  LOAD GAL",(char **)gb_list_gal_title,gb_gal_id_sel_cur,max_list_gal);
 }
 else
 {
  aSelNum = ShowTinyMenu("  LEER GAL",(char **)gb_list_gal_title,gb_gal_id_sel_cur,max_list_gal);
 }
 ...
 aSelNum = ShowTinyMenu("    RUN",((gb_language_en==1)?(char **)gb_bool_menu_en:(char **)gb_bool_menu_sp),0,max_gb_bool_menu);
 if (aSelNum==0)
 {
  gb_cont_pul_vkeys=0;
  gb_cont_vkeys=0;
...
void ShowTinyInterceptTapeMenu()
{
 ...
 if (gb_language_en==1)
 {
  aSelNum = ShowTinyMenu("  TAPE ROM", (char **)gb_bool_menu_en,pos,max_gb_bool_menu);
 }
 else
 {
  aSelNum = ShowTinyMenu(" CINTA ROM", (char **)gb_bool_menu_sp,pos,max_gb_bool_menu);
 }
 if (aSelNum == 255)
 {//Pulsa ESC vuelve a menu anterior Main menu
  gb_id_menu_cur= id_menu_main;
