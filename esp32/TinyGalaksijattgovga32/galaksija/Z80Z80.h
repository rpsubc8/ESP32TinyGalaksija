...
#define _Z80Z80_H
 ...
 #include "gbConfig.h"
 //se requiere little endian ya viene en gbConfig.h
 //#define LSB_FIRST
...
//*************************************************************
 ...
 //typedef unsigned short word;  //revisar redifinicion tipo
 //typedef unsigned short int word;
 typedef unsigned short int wordZ80;
#endif
...
{
#ifdef LSB_FIRST
  struct { unsigned char l,h; } B; //struct { byte l,h; } B;
#else
  struct { unsigned char h,l; } B; //struct { byte h,l; } B;
#endif
  wordZ80 W;
} pair;
...
typedef struct
{
  ...
  unsigned char IFF,I; //byte IFF,I;  // Interrupt registers
  unsigned char R; //byte R;          // Refresh register

  int IPeriod,ICount; // Set IPeriod to number of CPU cycles
                      // between calls to LoopZ80()
  int IBackup;        // Private, don't touch
  wordZ80 IRequest;      // Set to address of pending IRQ
  unsigned char IAutoReset; //byte IAutoReset;    // Set to 1 to autom. reset IRequest
  unsigned char TrapBadOps; //byte TrapBadOps;    // Set to 1 to warn of illegal opcodes
  wordZ80 Trap;          // Set Trap to address to trace from
  unsigned char Trace; //byte Trace;         // Set Trace=1 to start tracing
...
//*************************************************************
void IntZ80(register Z80 *R,register wordZ80 Vector);
...
//*************************************************************
#ifndef EXECZ80
unsigned short int RunZ80(register Z80 *R);
#endif
...
void WrZ80(unsigned short int Addr, unsigned char Value); //void WrZ80(register wordZ80 Addr,register byte Value);
unsigned char RdZ80(unsigned short int Addr); //byte RdZ80(register wordZ80 Addr);
...
//************************************ TO BE WRITTEN BY USER **
void OutZ80(unsigned short int Port,unsigned char Value); //void OutZ80(register wordZ80 Port,register byte Value);
unsigned char InZ80(unsigned short int Port); //byte InZ80(register wordZ80 Port);
...
//*************************************************************
#ifdef DEBUG
unsigned char DebugZ80(register Z80 *R);
#endif
...
//************************************ TO BE WRITTEN BY USER **
unsigned short int LoopZ80(register Z80 *R);
